...
const ChatMessageScreen = ({route, navigation}) => {
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          const fileObj =
            selectedImage || selectedVideo || selectedDocument || selectedAudio;
  ...
  useEffect(() => {
    ...
    const handleOutgoingMessage = async (data) => {
      console.log('New Outgoing Message:', data);
      ...
      if (data.conversationId === id) {
        ...
        console.log('Message not saved to Realm:', outgoingMessage);
    
        try {
          await upsertMessageToRealm(realm, outgoingMessage);
          console.log('✅ Outgoing message saved to Realm:', outgoingMessage);
        } catch (err) {
          console.error('Error saving outgoing message to Realm:', err);
        }
    
        setMessages((prevMessages) => [outgoingMessage, ...prevMessages]);
      }
    };
    ...
    socket.on('newOutGoingMessage', handleOutgoingMessage);
    socket.on('newIncomingMessage', handleNewMessage);
    socket.on('UpdateStatusEvent', handleStatusUpdate);
    ...
    return () => {
      socket.off('newIncomingMessage', handleNewMessage);
      socket.off('UpdateStatusEvent', handleStatusUpdate);
      socket.off('newOutGoingMessage', handleOutgoingMessage);
    };
  }, [id]);
